...
use crate::{loginmanager::{LogMngTrait, self, LoginManager}, models::User, routes};
#[derive(Clone)]
struct MockLogMngr{
...
impl LogMngTrait for MockLogMngr{
    ...
    fn get_users_list(&self) -> Result<Vec<User>, diesel::result::Error>{
        ...
        for (log,pass) in iterat{
            res.push(User{login:log.to_string(),password:pass.to_string()});
        }
        Ok(res)
    }
...
async fn login_route_test() {
    ...
    let mngr=MockLogMngr{inner:Arc::new(RwLock::new(BTreeMap::new()))};
    let test_stuct=User{login:"123".to_string(),password:"321".to_string()};
    ...
    assert_eq!(req_test.headers().get("token").unwrap().to_str().unwrap(),LoginManager::get_security_key());
...
async fn get_users_route_test() {
    ...
    let mngr=MockLogMngr{inner:Arc::new(RwLock::new(BTreeMap::new()))};
    let test_stuct=User{login:"123".to_string(),password:"321".to_string()};
    let test_struct=User{login:"ABC".to_string(),password:"CBA".to_string()};
    mngr.insert_new_user(test_stuct.clone());
